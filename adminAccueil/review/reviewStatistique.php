...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #72C938; color: #fff;">
        ...
        ?>
            <p>Revues total <br> Disponible</p>
        </div>
        <div class="icon">
        <img src="../icons/bookshelf.png" alt="Votre Icône" />
        </div>
        <a href="review.php" class="small-box-footer">Plus d'info <i class="fa fa-arrow-circle-right"></i></a>
    </div>    
</div>
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38C946; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_consultation";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Revues total <br> consultés </p>
        </div>
        <div class="icon">
        <img src="../icons/consultant.png" alt="Votre Icône" />
        </div>
        <a href="../review/review_consultation.php" class="small-box-footer">Plus d'info <i class="fa fa-arrow-circle-right"></i></a>
    </div>    
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38C98F; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_borrow";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Revues Total <br> empruntées</p>
        </div>
        <div class="icon">
        <img src="../icons/book.png" alt="Votre Icône" />
        </div>
        <a href="../review/review_borrow.php" class="small-box-footer">Plus d'info <i class="fa fa-arrow-circle-right"></i></a>
    </div>    
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38BBC9; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_borrow WHERE status IS NULL";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Total de Revue <br> en emprunts</p>
        </div>
        <div class="icon">
        <img src="../icons/book.png" alt="Votre Icône" />
        </div>
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38BBC9; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_consultation WHERE status IS NULL";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Total de Revues <br> en consultation</p>
        </div>
        <div class="icon">
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38C98F; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_borrow WHERE borrow_review_date = CURDATE()";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Total de Revues <br> empruntés aujourd'hui</p>
        </div>
        <div class="icon">
...
<div class="col-lg-3 col-xs-6">
    ...
    <div class="small-box" style="background-color: #38C946; color: #fff;">
        <div class="inner">
        <?php
        $sql = "SELECT * FROM review_consultation WHERE consultation_review_date = CURDATE()";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$query->num_rows."</h3>"
        ?>
            <p>Total de Revues <br> consultés aujourd'hui</p>
        </div>
        <div class="icon">
        <img src="../icons/reader.png" alt="Votre Icône" />
        </div>
    ...
    <div class="small-box" style="background-color: #72C938; color: #fff;">
      ...
      <?php
        ...
        $sql = "SELECT COUNT(*) AS total FROM (
          SELECT * FROM review_borrow WHERE DATE(return_review_date) = '$date'
          UNION ALL
          SELECT * FROM review_consultation WHERE DATE(return_review_date) = '$date'
        ) AS combined_table";
        $query = $conn->query($sql);
        $row = $query->fetch_assoc();
        echo "<h3>".$row['total']."</h3>";
      ?>
        <p>Total de Revues <br> retournés aujourd'hui</p>
      </div>
      <div class="icon">
...
<div class="col-lg-12 col-xs-7">
  <h3 class="card-title">STATISTIQUE SUR LES REVUES PAR FACULTE</h3>
    <div class="card-group">
        <?php
            $sql = "SELECT faculty.name_faculty, COUNT(review.review_id) AS num_review
                    FROM faculty
            ...
            while ($row = $query->fetch_assoc()) 
            {
                ...
                echo "<p class='card-text'>".$row['num_review']." Revues</p>";
                echo "</div>";
    ...
    <div class="card-group">
        <?php
            $sql = "SELECT review_category.name_category_review, COUNT(review.review_id) AS num_review_cat
            ...
            while ($row = $query->fetch_assoc()) 
            {
                ...
                echo "<p class='card-text'>".$row['num_review_cat']." Revues</p>";
                echo "</div>";
                echo "<div class='card-footer'>";
                echo "</div>";
                echo "</div>";
...
<style>
    ...
    .card {
      ...
      margin-right: 6px;
        ...
        border-radius:10px;
        margin-top: 10px;
        width: 19%; /* Ajustez la largeur des cartes selon vos préférences */
        margin-bottom: 10px;
    }
    .card-text{
      font-size :20px;
...
$borrow_year = 'AND YEAR(borrow_review_date) = ' . mysqli_real_escape_string($conn, $year);
$consult_year = 'AND YEAR(consultation_review_date) = ' . mysqli_real_escape_string($conn, $year);
$borrow = array();
$consultation = array();
$num_men = array();
$num_women = array();
...
for ($m = 1; $m <= 12; $m++) {
  ...
  $sql = "SELECT * FROM review_borrow WHERE MONTH(borrow_review_date) = '$m' $borrow_year";
  $rquery = $conn->query($sql);
  array_push($borrow, $rquery->num_rows);

  // Livres consultés par mois
  $sql = "SELECT * FROM review_consultation WHERE MONTH(consultation_review_date) = '$m' $consult_year";
  ...
  array_push($consultation, $men_query->num_rows);
    ...
    COUNT(DISTINCT review_borrow.borrow_review_id) AS num_borrows,
    COUNT(DISTINCT review_consultation.consultation_review_id) AS num_consultations
    FROM reader_category
    LEFT JOIN reader ON reader_category.reader_category_id = reader.reader_category_id
    LEFT JOIN review_borrow ON reader.reader_id = review_borrow.reader_id AND MONTH(review_borrow.borrow_review_date) = '$m' $borrow_year
    LEFT JOIN review_consultation ON reader.reader_id = review_consultation.reader_id AND MONTH(review_consultation.consultation_review_date) = '$m' $consult_year
