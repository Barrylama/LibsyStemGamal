...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Consultation Memoires
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Accueil</a></li>
        <li>Transactions</li>
        <li class="active">Consultations Memoires</li>
      </ol>
    </section>
    <!-- Main content -->
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    ...
                    memory_consultation.consultation_memory_id AS consultationMemoryId, 
                    memory_consultation.status AS barstat, 
                    IF(memory_consultation.status = 1, DATE_FORMAT(memory_consultation.return_consultation_date, '%d,%M, %Y à %H:%i'), 'Non retourné') AS return_date_display
                    FROM memory_consultation
                    ...
                    ORDER BY consultation_memory_date DESC";
                    ...
                    while ($row = $query->fetch_assoc()) {
                        ...
                        echo "
                            <tr>
                                <td>" . date('d,M, Y', strtotime($row['consultation_memory_date'])) . "</td>
                                <td>" . $row['readId'] . "</td>
...
function getRow(id){
  $.ajax({
    ...
    data: {consultation_memory_id: id},
    dataType: 'json',
    success: function(response){
      // Récupération de l'ID
      $('.consultationMemoryId').val(response.consultationMemoryId);
      console.log(response.consultationMemoryId);

      // Affichage du titre du livre dans la modal
      $('.return_book').html(response.memory_theme);
    }
  });
}
