	...
	if(isset($_POST['add'])){
		...
		if($query->num_rows < 1){
			...
			foreach($_POST['isbn'] as $isbn){
				if(!empty($isbn)){
					...
					if($query->num_rows > 0){
						...
						$sql = "INSERT INTO memory_consultation (reader_id,memory_id, consultation_memory_date) VALUES ('$student_id', '$bid', NOW())";
						if($conn->query($sql)){
							$added++;
						...
						$_SESSION['error'][] = 'La memoire avec le NUMERO - '.$isbn.' n\'est pas disponible';
					}
				}
			}
			...
			if($added > 0){
				...
				$memorys = ($added == 1) ? 'mis en consulation avec succès' : 'misent en consulation avec succès';
