...
function Header()
{
    ...
    $this->Cell(0, 10, 'Liste des lecteurs venants de l\'exterieur de Gamal', 0, 1, 'C');
    // Total des étudiants du centre informatique
    $this->SetFont('Arial', 'B', 9);
    $this->Cell(0, 2, 'Total des lecteurs : ' . $this->totalStudents, 0, 1, 'C');
    $this->Image('fpdf/tutorial/logoci.png', 173, 5, 30);
    // Saut de ligne
    $this->Ln(10);

    // Titres des colonnes
    $header = array('PRENOMS', 'NOMS', 'NUMERO CIDN', 'PROFESSION', 'ADRESSE', 'TELEPHONES');
    $this->SetFont('Arial', 'B', 10);
    $this->Cell(51, 10, $header[0], 1, 0, 'C');
    $this->Cell(18, 10, $header[1], 1, 0, 'C');
    $this->Cell(30, 10, $header[2], 1, 0, 'C');
    $this->Cell(33, 10, $header[3], 1, 0, 'C');
    $this->Cell(33, 10, $header[4], 1, 0, 'C');
    $this->Cell(30, 10, $header[5], 1, 1, 'C');}
...
WHERE reader_category.reader_category_id = 22");
$stmt->execute();
...
foreach ($data as $row) {
    ...
    $pdf->Cell(30, 7, $row['Matricule'], 1, 0, 'C');
    $pdf->Cell(33, 7, $row['Poste'], 1, 0, 'C');
    $pdf->Cell(33, 7, $row['reader_address'], 1, 0, 'C');
    $pdf->Cell(30, 7, $row['reader_phone'], 1, 1, 'C');
}
