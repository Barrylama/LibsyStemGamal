...
function Header()
{
    ...
    $this->Cell(0, 10, 'Liste des lecteurs appertenant au personnel universitaire', 0, 1, 'C');
    ...
    // Titres des colonnes
    $header = array('PRENOMS', 'NOMS', 'FACULTE', 'POSTE', 'ADRESSE', 'TELEPHONES');
    $this->SetFont('Arial', 'B', 10);
    $this->Cell(51, 10, $header[0], 1, 0, 'C');
    $this->Cell(18, 10, $header[1], 1, 0, 'C');
    $this->Cell(32, 10, $header[2], 1, 0, 'C');
    $this->Cell(35, 10, $header[3], 1, 0, 'C');
    $this->Cell(30, 10, $header[4], 1, 0, 'C');
...
foreach ($data as $row) {
    ...
    $pdf->Cell(32, 7, $row['Departement'], 1, 0, 'C');
    $pdf->Cell(35, 7, $row['Poste'], 1, 0, 'C');
    $pdf->Cell(30, 7, $row['reader_address'], 1, 0, 'C');
