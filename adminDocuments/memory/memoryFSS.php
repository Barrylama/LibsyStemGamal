...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
      Memoires de la faculté des Sciences et techniques de la santé 
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Accueil</a></li>
        <li>Memoires</li>
        <li class="active">Sciences et techniques de la santé</li>
      </ol>
    </section>
    <!-- Main content -->
                    ...
                    echo "
                        <tr>
                          ...
                          <td>
                            ...
                            <button class='btn btn-success btn-xs edit btn-flat' data-id='" . $row['memoryId'] . "'><i class='fa fa-edit'></i></button>
                            <button class='btn btn-danger btn-xs delete btn-flat' data-id='" . $row['memoryId'] . "'><i class='fa fa-trash'></i></button>
                          </td>
                        </tr>
                    ";
                }
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      ...
      $('#sel_faculte_memory').val(response.faculty_id).html(response.name_faculty);
      $('#sel_category_memory').val(response.memory_category_id).html(response.name_category_memory);
      $('#edit_isbn').val(response.memory_number);
      $('#edit_titre').val(response.memory_theme);
      $('#edit_author').val(response.memory_author);
      $('#edit_datepicker_add').val(response.memory_publish_date);
      $('#edit_NBexemplaire').val(response.NbExemplaireTotal);
      $('#edit_description').val(response.memory_description);
      ...
      $('.del_memory').html(response.memory_theme);
    }
  });
}
</script>
