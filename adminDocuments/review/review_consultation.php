  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li>Transactions</li>
        <li class="active">Consultations Revues</li>
      </ol>
    </section>
    <!-- Main content -->
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    ...
                    review_consultation.consultation_review_id AS consultationReviewId, 
                    review_consultation.status AS barstat, 
                    IF(review_consultation.status = 1, DATE_FORMAT(review_consultation.return_review_date, '%d,%M, %Y à %H:%i'), 'Non retourné') AS return_date_display
                    FROM review_consultation
                    ...
                    while ($row = $query->fetch_assoc()) {
                        ...
                        $returnButton = ($row['barstat']) ? '' : '<button class="btn btn-success btn-xs return-btn" data-book-id="' . $row['consultation_review_id'] . '">Retourner</button>';
                        ...
                        echo "
                            <tr>
                                <td>" . date('d,M, Y', strtotime($row['consultation_review_date'])) . "</td>
                                <td>" . $row['readId'] . "</td>
...
function getRow(id){
  $.ajax({
    ...
    data: {consultation_review_id: id},
    dataType: 'json',
    success: function(response){
      // Récupération de l'ID
      $('.consultationReviewId').val(response.consultationReviewId);
      console.log(response.consultationReviewId);
