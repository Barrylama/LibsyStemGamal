	...
	if(isset($_POST['add'])){
		...
		if($query->num_rows < 1){
			...
			foreach($_POST['isbn'] as $isbn){
				if(!empty($isbn)){
					...
					if($query->num_rows > 0){
						...
						$sql = "INSERT INTO review_borrow (reader_id,review_id, borrow_review_date) VALUES ('$student_id', '$bid', NOW())";
						if($conn->query($sql)){
							$added++;
			...
			if($added > 0){
				...
				$reviews = ($added == 1) ? 'empruntée avec succès' : 'empruntées avec succès';
